...
program
  ...
  .option(
    "--api-key <key>",
    "Vercel AI Gateway API key (overrides AI_GATEWAY_API_KEY)",
  )
  .option(
    "--image-model <model>",
    "Model for image generation (default: google/gemini-2.5-flash-image)",
    "google/gemini-2.5-flash-image",
  )
  .option(
    "--generate-copy",
    "Generate ad copy (headline + CTA) via AI instead of using campaign message",
  )
  .option(
    "--copy-model <model>",
    "Model for AI ad copy (overrides AMPLIFY_TEXT_MODEL)",
  )
  .option(
    "--enhance-images",
    "Enhance provided product images via img-to-img AI generation",
  )
  .option(
    "--overlay-mode <mode>",
    "Force render mode for all products: placement or overlay",
  )
  .option(
    "--templates <ids>",
    "Comma-separated template IDs (square,story,landscape)",
  ...
  .action(
    async (
      ...
      try {
        ...
        console.log(
          chalk.dim(
            `  Products: ${brief.products.map((p) => p.name).join(", ")}`,
          ),
        );
        if (brief.branding) {
        ...
        const copyModel = generateCopy
          ? (options.copyModel ?? config.textModel)
          : undefined;
        const format = (
          options.format === "webp" ? "webp" : "png"
        ) as OutputFormat;
        ...
        if (options.overlayMode) {
          if (
            options.overlayMode !== "placement" &&
            options.overlayMode !== "overlay"
          ) {
            console.error(
              chalk.red("Error:"),
              "--overlay-mode must be 'placement' or 'overlay'",
            );
            process.exit(1);
          }
        ...
        if (enhanceImages && !apiKey) {
          console.log(
            chalk.yellow("⚠ --enhance-images requires an API key, ignoring"),
          );
        }
        ...
        const resolved = await resolveAllAssets(brief, {
          generator: apiKey ? { model: imageModel } : undefined,
        ...
        const result = await composeCreatives(brief, resolved, {
          ...
          copywriter: generateCopy ? { model: copyModel } : undefined,
