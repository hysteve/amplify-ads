 ...
 */
export async function generateProductImage(product, context, options) {
 ...
 */
export async function enhanceProductImage(product, sourceImagePath, context, options) {
    const colorHints = buildColorHints(context.branding);
    const styleHint = context.branding?.style
        ? `Visual style: ${context.branding.style}.`
        : "";
...
function buildColorHints(branding) {
    if (!branding)
        return "";
    const parts = [];
